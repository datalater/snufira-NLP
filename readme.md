...
word2vec과 매우 유사한데 단어를 더 잘게 쪼갠다.
단어를 더 잘게 쪼개면 rare한 단어를 잘 처리하게 된다.
...
1. word2vec, FastText 두 모델 비교 평가
2. 누가 최적의 모델을 찾아왔는지 평가할 것
3. 코드 + PT용 파일 (PT는 기말고사 이후 몇 팀만 하게 됨)

기한
- 기말과제, 기말고사 11/29(수)
- PT 내용은 우리는 어떻게 모델을 짰다 이런 걸 쓰면 됨
- 기말고사는 저번 시험 이후부터 11/22까지 배운 내용
```
